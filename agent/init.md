...
- **本地/通用环境**：工作文件根默认 `盯盘/`；记忆位于 `盯盘/agent记忆/`；业务文档缓存位于 `盯盘/工作文档/`（含 `index.md`、`agents/`、`skills/`、`接口文档/`，工作产物，初始化与任务启动时从工程 `agent/`、`doc/` 同步落地后读取，不写入记忆）；普通临时产物位于 `盯盘/tmp/tmp_YYYYMMDD-HHmmss_文件名`。
- **Coze**：左侧“工作文件”放 `工作文档/`（业务文档缓存：index、agents、skills、接口文档）、日报、投研、选股、自动任务报告、复盘及 `tmp/`；右侧“记忆”放 `基础设定/SOUL.md` 和 `所有对话/主对话/` 下的 `MEMORY.md`、`USER.md`、`关注与持仓.md`、`服务状态与能力.md`，其中 `recent_memory/` 等价于逻辑 `短期记忆/`。
...
### v2.7.0 当前版本执行约束（最高优先，覆盖旧版任务编号）

1. **交易日定时任务只保留两个**：**T1（08:30 盘前汇总，团队）** 与 **T3（22:00 综合复盘 + 正式选股 + 回测，团队）**。**取消 T2（17:30 当日总结）**。周期任务 W1/M1/P1 不变。
2. **旧编号一律按新集合执行**：任何文档、模板、角色、Skill 中若仍出现把 T2（17:30 当日总结）当作现行任务的三任务枚举，一律按 **T1/T3** 执行；「只读日终状态」「延迟重试」等原 T2/T3 规则按 **T3** 执行；「遇非交易日返回」按 **T1/T3** 执行。初始化时删除任何已注册的 T2/17:30 当日总结任务。
3. **日报目录路径同步重编号**：交易日日报目录 `盯盘/yyyy年MM月dd日/` 固定只含 **`01-盘前汇总.md`**（T1）与 **`02-综合复盘.md`**（T3）。原 `04-当日总结.md` 取消；原 `05-综合复盘.md` 重编号为 `02-综合复盘.md`。所有引用同步更新。
4. 复盘（T3）继续按「热点→申万分级行业→行业内量化→逐只理由→上传服务端」产出正式候选；说人话、结论前置、接口问题置于报告文末等 v2.5.0/v2.6.0 规则不变。

### v2.6.0 当前版本执行约束（最高优先，覆盖旧版冲突项）

> 核心原则：**一切业务性质文档都要落地到 agent 自身工作目录再读取；全数据链路都要有明确的文档路径与接口说明指引，任何地方都不许说明不明确到让 agent 靠猜；每次涉及工作文档 / 定时任务 / 初始化功能的改动，都要把改动点全部更新、重新加载到工作目录与定时任务日程，并同步到记忆与 SOUL，只保留最新版本快照，不罗列历史版本。**

#### 工作目录文档缓存结构（本地/通用 `盯盘/工作文档/`，Coze 左侧工作文件 `工作文档/`）

```text
盯盘/工作文档/                      # 业务文档缓存（从工程 agent/、doc/ 同步落地；工作产物，不进记忆区）
├── index.md                        # 强制执行索引（源 agent/index.md）
├── SOUL.md 只读引用                # 人格铁律仍以记忆区 SOUL 为准，此处不复制
├── agents/                         # 主 + 5 子 Agent 定义、TEAM.md、ORCHESTRATION.md（源 agent/agents/）
├── skills/                         # 12 个 SKILL.md（源 agent/skills/*/SKILL.md）
└── 接口文档/
    ├── AGENT_SERVICE_GUIDE.md      # 调用协议 / 五轨版本 / 错误码（源 doc/AGENT_SERVICE_GUIDE.md）
    └── SERVICE_INDEX.md            # 功能分组索引与 Skill / 定时任务交叉表（源 doc/SERVICE_INDEX.md）
```

1. **业务文档必须全量落地工作目录再读取**：初始化及每次任务/角色启动时，先把上表全部内容（子 Agent 定义、12 个 Skill、接口文档、执行索引）从工程目录同步到工作目录缓存，并**从工作目录读取内化**；不得凭记忆、角色摘要或旧缓存代替正文。任一文件未落地或读取失败，立即停止依赖它的动作并如实报告。缓存是工作产物，不写入记忆区。
2. **接口规范就近附带**：每个使用接口的 Skill、子 Agent 定义与定时任务，都必须在文档内附「本环节接口速查」——列出所用功能名 + 一句话用途 + 关键参数要点，并指向工作目录接口文档路径 `工作文档/接口文档/AGENT_SERVICE_GUIDE.md`、`工作文档/接口文档/SERVICE_INDEX.md`，使 AI 随时可查、无需猜测。
3. **定时任务描述结构化**：加载完文档后，Agent 依据工作目录文档路径生成每个定时任务的描述，任务描述必须含四段——【工作目标】【结果存放的目录结构】【Agent 编排（主/子 Agent 分工）】【使用的 Skill 和接口文档/索引，标注其内化到工作目录的路径】。详见 `schedule.md`。
4. **报告中数据接口问题一律置于文末**：定时任务与 Skill 生成的报告，正文只写面向用户的结论；接口调用相关问题（失败、降级、缺数）统一收敛到报告末尾的「数据接口问题」附录，逐条标明**接口名称、功能、报错信息**；**仅量化选股（`screen_quant`/`screen_trend`/`screen_sector`）与选股上传（`log_selection`）接口需要附请求参数**，其余接口不附参数。正文首屏严禁出现裸接口名、参数名与原始返回。
5. **版本控制本地优先、按改动类型重载**：文档版本对齐时**优先读取本地工程文件（`agent/`、`doc/` 及工作目录缓存），本地读不到再从 git（`git show` / GitHub raw）拉取**；子 Agent 定义 / Skill / 接口文档 / 索引有更新时，必须重新同步落地到工作目录缓存；定时任务定义有更新时，必须重设（删除旧的再注册新的）定时任务日程。版本状态只在记忆保留最新快照，不罗列历史版本。
6. **必读铁律附索引**：SOUL 与主记忆保留「查阅路由 / 文档地图」索引，指向工作目录缓存中 index、agents、skills、接口文档的位置；完成加载后 Agent 在 `服务状态与能力.md` 记录「工作目录文档地图」（缓存根路径 + 各类文档落地路径 + 同步时间 + 对应 `agent_doc_version`），便于随时定位所需功能与规范。

（v2.5.0 的四项硬门槛继续生效：技能落地工作目录、定时任务中间产物不留存、报告说人话与结论前置、热点→申万分级行业量化选股并上传服务端。原 `盯盘/skills/` 统一并入 `盯盘/工作文档/skills/`。）
...
1. **仅注册现行任务**（以 v2.7.0 为准）：只允许注册 `schedule.md` 中的 T1、T3、W1、M1、P1；其中 T1 是 08:30 盘前汇总，T3 是 22:00 综合复盘。**T2（17:30 当日总结）已取消**。任何现行规范、模板、角色与 Skill 出现 T1/T3 时按 T1/T3 执行。
2. **初始化必须清理旧任务**：删除旧 T6、旧 T7、旧 D1、旧 T2/T3/T4/T5、**已取消的 T2（17:30 当日总结）**，以及任何自动竞价、自动盘中盯盘、午间总结、Agent 预计算任务；不得迁移为其他自动任务。
3. **服务端负责 16:00 日终收口**：数据服务在交易日 16:00 自动补齐全市场行业与个股因子。Agent 不注册、不触发、不补跑，也不得在失败、缺数或状态异常时调用 `precompute_daily_factors`。
4. **Agent 只读日终状态**：T3 通过 `health.daily_finalize` 与 `precompute_status` 核验日终状态；状态未成功时披露缺口并按报告接口规则重试读取，禁止把状态读取失败升级为预计算调用。
...
4. 增量路径获取变动文件内容时**本地优先**：先读本地工程文件（`agent/`、`doc/` 及工作目录缓存 `工作文档/`）；本地读不到或不可用时，再按目标 `git_revision` 从 git 拉取——`git show <git_revision>:<path>`，仍不可用回退 GitHub raw `https://raw.githubusercontent.com/seksibich/asking_agent_team/<git_revision>/<path>`，只拉变更清单文件。
5. **按改动类型执行重载**：变更清单涉及子 Agent 定义 / Skill / 接口文档 / 索引时，必须把更新后的文件重新同步落地到工作目录缓存 `工作文档/`，并从工作目录重读内化；涉及定时任务定义（`schedule.md`）时，必须重设（删旧注新）对应定时任务日程，并按结构化模板重新生成任务描述。
6. **改动全覆盖 + 只留最新快照**：一次升级要把所有变更点全部内化并同步到记忆与 SOUL 的查阅路由；记忆中版本状态、文档地图只保留最新一份快照，不罗列历史版本（历史仅在 `profile/CHANGELOG-AGENT.md` 保留）。
...
### 第 3 步：全量落地业务文档到工作目录并完整读取（强制，先落地再读取）
首次初始化以及**每次任务启动、每次 Agent/角色启动**时，必须**先把全部业务文档从工程目录同步到工作目录缓存**（本地/通用 `盯盘/工作文档/`，Coze 左侧工作文件 `工作文档/`），再从工作目录逐文件完整读取内化；不得只凭 `index.md`、角色摘要、历史记忆或主绑定清单代替正文。缓存是工作产物，不写入记忆区；文档版本变化时先刷新缓存再执行任务。落地清单：

- `工作文档/index.md` ← `agent/index.md`（执行索引）
- `工作文档/agents/` ← `agent/agents/`（`TEAM.md`、`ORCHESTRATION.md`、主 + 5 子 Agent 定义）
- `工作文档/接口文档/AGENT_SERVICE_GUIDE.md` ← `doc/AGENT_SERVICE_GUIDE.md`（调用协议 / 五轨版本 / 错误码）
- `工作文档/接口文档/SERVICE_INDEX.md` ← `doc/SERVICE_INDEX.md`（功能分组索引与交叉表）
- `工作文档/skills/` ← 固定 12 个 `agent/skills/<name>/SKILL.md`，即：
  1. `priority-framework` 2. `data-service` 3. `output-format` 4. `pre-market` 5. `bidding-analysis` 6. `intraday-watch` 7. `post-market` 8. `industry-analysis` 9. `stock-screening` 10. `quant-screening` 11. `review-learning` 12. `stock-research`

完整加载后再读取 `memory/MEMORY.md` 与 `memory/PORTFOLIO.md`，并按任务读取运行期专项记忆。角色文件中的「主绑定」只表示该角色本次优先执行的 Skills，**不减少完整加载范围**；任一文件缺失或无法读取时须报告并停止依赖该文档的动作，禁止使用简化描述猜测规则。SOUL 人格铁律仍以记忆区 `SOUL.md` 为准，不复制进工作目录。
...
- `服务状态与能力.md`：BASE_URL、其他 health 版本、Agent 文档同步记录、功能索引、标签及接口约定；并新增「工作目录文档地图」——记录工作目录缓存根路径、index / agents / skills / 接口文档各自落地路径、同步时间与对应 `agent_doc_version`，只保留最新一份快照，便于随时定位所需功能与接口规范。
...
### 第 6 步：加载 Agent 团队
从工作目录缓存 `工作文档/agents/` 读取 `TEAM.md`、`ORCHESTRATION.md` 与各角色文件（已在第 3 步落地）。明确：团队用于盘前汇总（T1）、综合复盘（T3）、周/月回测、用户分析。竞价、盘中扫描和午间总结不设自动任务，仅在用户明确请求时由主 Agent 单次执行。
...
### 第 8 步：注册定时任务（描述结构化，引用工作目录路径）
读取工作目录缓存中的 `schedule.md`（源 `agent/schedule.md`），先删除旧 T6/T7/D1、历史 T2/T3/T4/T5、**已取消的 T2（17:30 当日总结）**，以及所有自动竞价、盘中扫描、午间总结、Agent 预计算任务，再且仅注册 **T1/T3/W1/M1/P1**。**每个任务的描述由 Agent 依据已落地的工作目录文档路径生成，必须含四段：【工作目标】【结果存放的目录结构】【Agent 编排（主/子 Agent 分工）】【使用的 Skill 和接口文档/索引及其工作目录内化路径】**（见 `schedule.md` 的结构化模板）。禁止创建调用 `bidding_analysis`、`watch_intraday`、`precompute_daily_factors` 的调度器、Hook、cron 或 Agent 循环；服务端交易日 16:00 日终收口不由 Agent 注册。定时任务定义有更新时必须重设（删旧注新）任务日程。

### 第 9 步：初始化回执
输出确认：文档版本与 `git_revision`、人格与团队、12 个 Skills、分析重心、服务连通与 `data_version`、`MEMORY.md` / `USER.md` / 服务状态 / 持仓镜像 / 短期目录状态、已删除的过期事项及附件；**确认业务文档已全量落地工作目录缓存 `工作文档/`（index、agents、skills、接口文档）并已在 `服务状态与能力.md` 记录「工作目录文档地图」**；确认 T1/T3/W1/M1/P1 已按结构化模板（工作目标 / 结果目录结构 / Agent 编排 / Skill 与接口文档路径）生成任务描述；明确报告已删除旧 T6/T7/D1、历史 T2/T3/T4/T5、**已取消的 T2（17:30 当日总结）**及旧 Agent 自动盯盘/预计算任务，仅保留 **T1/T3/W1/M1/P1**，并确认服务端 `quant_watch` 与 Agent 手动盯盘职责隔离、日终只读 `health.daily_finalize` / `precompute_status`。
...
5. 普通临时脚本、文档和转换产物只写工作文件根 `tmp/tmp_YYYYMMDD-HHmmss_文件名`，不得进入记忆；Coze 严格区分左侧工作文件与右侧记忆。定时任务中子 Agent 的原始意见、逐接口返回、草稿和中间打分表也只写 `tmp/`，任务结束后立即清理，只保留主 Agent 汇总的最终报告与规则允许的业务快照。
...
- 每次任务与每个角色启动都重新完整读取第 3 步列出的全部 12 个 `SKILL.md`；角色主绑定不是免读清单。`stock-research` 不加入定时 T1/T3 的必执行绑定。
- 任务分发按 `agents/TEAM.md` 矩阵点名 `skills/<name>/SKILL.md`，并执行 `skills/data-service/SKILL.md` 的统一 fallback、缺失标注与 T1/T3 延迟重试。
- **报告与推送分离**：T1/T3 Markdown 在可核验范围内尽可能详尽，固定为“一眼结论（核心摘要）→目录导读→详细正文”；首屏先给仓位/次日倾向、题材/具体事件 Top N、“题材/事件 → 个股”、最大风险/证伪及首屏结论表。数据不可用仍保留章节并披露 fallback、实际日期与缺失项。
...
- **报告首屏说人话、放重点**：面向用户的报告（T1/T3、竞价、早盘、周报、月报、选股与调研）标题后第一节固定为 `## 🎯 一眼结论（核心摘要）`，先答「该做什么、盯什么、怕什么」——📊 仓位/倾向、🔥 题材/事件、🎯 关注个股/候选、⚠️ 风险/证伪；复盘总结、关键消息面、选股/持仓/关注结论一律前置，数据口径、来源清单、原始参数靠后。
