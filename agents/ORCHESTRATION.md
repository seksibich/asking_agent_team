...
## 场景二：17:30 当日总结（T6，主 Agent）

```
>> 前置：health/版本；读取 daily/yyyyMMdd.md、观察池和持仓/关注
...
>> 生成 04-当日总结.md：核心摘要 → 目录导读 → 数据状态 → 指数成交 →
   板块趋势与主线阶段 → 情绪/连板/极端指数 → 资金 → 涨价链 →
   持仓关注 → 明日环境初判 → 风险 → 来源
...
>> 并行分发（5 子 Agent + 回测）：
   ...
   -> 基本面研报：
        A. 涨价链/景气进展与次日预期（price_hike_scan/news_filter）
        B. 业绩窗口判定；窗口内调用 fundamental_forecast/fundamental_express/news_anns，
...
>> 业绩增长参考池（独立支线）：
   ...
   · 不 log_selection、不写 predictions/观察池、不进 auto/watch/holding、不参与回测调参
...
>> 合并输出 05-综合复盘.md：核心摘要 → 目录导读 → 数据状态 → 全天行情 →
   板块趋势/主线 → 情绪与连板 → 资金龙虎榜 → 涨价景气 → 晚间公告 →
   正式量化/趋势候选 → 业绩增长参考池 → 明日策略 → 回测调参 → 风险 → 来源
>> 报告详尽且不受推送字数限制；推送另按 ≤500 字重点模板生成
```

### T1 同步要求

T1 场景一的合并输出同样执行 `skills/output-format/SKILL.md`：`01-盘前汇总.md` 必须是「核心摘要→目录导读→详细正文」，至少含数据状态、隔夜/宏观/消息、涨价景气、指数与板块趋势、情绪温度/极端指数/择时、近7日选股复核、今日主线、正式关注完整理由、持仓关注、风险、来源；推送独立且建议不超过 500 字。
...
## v1.1.0 强制 Skill 分发与 fallback（覆盖上方简写）

### 所有场景前置

每次任务以及每个角色启动时，先完整读取固定 11 个 `skills/<name>/SKILL.md`；下列主绑定只决定本场景职责，不允许以本编排摘要替代 Skill 正文。所有取数统一执行 `skills/data-service/SKILL.md`，评分执行 `skills/priority-framework/SKILL.md`，报告执行 `skills/output-format/SKILL.md`。
...
### 17:30/22:00 盘后与复盘分发（T6/T7）

- T6 主 Agent：`skills/post-market/SKILL.md` + `skills/data-service/SKILL.md` + `skills/priority-framework/SKILL.md` + `skills/output-format/SKILL.md`。
- T7 主 Agent：在 T6 绑定上增加 `skills/industry-analysis/SKILL.md`、`skills/stock-screening/SKILL.md`、`skills/quant-screening/SKILL.md`、`skills/review-learning/SKILL.md`。
...
T1/T6/T7 关键接口 4xx/5xx/空数据先记录，5 分钟、15 分钟各延迟重试一次；401/参数或配置错误不盲目重试。`market_index` 失败/空/部分缺失时逐 code 调 `market_daily(code,start,end)` 最近记录并标 `degraded`/实际日期。`news_flash` 402 时用 `news_filter(keyword)+news_cctv+外部搜索`；同源失败继续 cctv+至少两个可信外部来源；全失败标“消息面不可用”，不得解释为无风险。关键源最终失败后按 fallback 降级并继续可完成部分，非关键接口失败不阻塞整份报告，禁止编造。