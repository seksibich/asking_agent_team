...
Python 解释器、虚拟环境和 `requirements.txt` 依赖由部署配置方按目标系统自行提供；本仓库不内置 Python 运行时或第三方依赖包。完成配置后，在仓库根目录执行：
...
如果使用虚拟环境，请先由配置方创建并激活 `.venv`；如果使用系统包管理器、内部镜像或离线制品库，也由配置方负责安装、版本匹配和安全校验。目标环境至少应满足 Python 3.11，并与 `requirements.txt` 兼容。
...
要求：由配置方提供 Python 3.11+、虚拟环境和已安装且通过校验的 `requirements.txt` 依赖。本仓库不负责提供或安装 Python 运行时及第三方依赖。
