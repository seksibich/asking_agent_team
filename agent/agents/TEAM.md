...
- **盯盘用主 Agent 单跑**：盘中每 10 分钟扫描、12:50 盘中消息+早盘总结、竞价、17:30 当日总结，均由主 Agent 独立完成，不启用团队（追求时效、避免开销）。
...
主 Agent 接到任务
  ...
  │
  └─→ 输出最终结果 + 写记忆（predictions / 观察池 / 学习日志 / 当日观察对象）
```
...
3. 一切数据经本地数据服务 `POST /call` 获取，标注 `source` 与 `fetched_at`；遵守版本机制。
