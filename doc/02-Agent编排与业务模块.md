...
- **技能落地工作目录**：初始化与每次任务/角色启动时，先把 12 个 `SKILL.md` 同步到工作目录技能缓存（`盯盘/skills/`，Coze 左侧 `skills/`）再读取内化；技能缓存是工作产物，不写入记忆，文档版本变化时刷新。
- **中间产物不留存**：定时任务（T1/T2/T3/W1/M1/P1）中，子 Agent 的原始意见、逐接口返回、草稿、中间打分表和临时资讯原文只写工作文件根 `tmp/`，任务结束即清理；只有主 Agent 汇总的最终报告与规则允许的业务快照（`daily`、`log_selection`、predictions、学习日志）可以保存。
