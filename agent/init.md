...
- **本地/通用环境**：工作文件根默认 `盯盘/`；记忆位于 `盯盘/agent记忆/`；技能缓存位于 `盯盘/skills/`（工作产物，初始化与任务启动时从仓库 `agent/skills/*/SKILL.md` 同步落地后读取，不写入记忆）；普通临时产物位于 `盯盘/tmp/tmp_YYYYMMDD-HHmmss_文件名`。
...
- **AGENT_DOC_VERSION：`v2.5.0`**（技能落地工作目录、子 Agent 中间产物不留存、报告说人话与结论前置、热点→申万分级行业量化选股并上传服务端，2026-07-21）
...
### v2.5.0 当前版本执行约束（最高优先，覆盖旧版冲突项）

> 本版本聚焦四件事：**技能真正落地到工作目录再读取**、**定时任务中间产物一律不留存**、**报告说人话且结论前置**、**按当日热点经申万分级行业量化选股并上传服务端**。四项是硬门槛，与旧版冲突时以本版本为准。

1. **技能必须落地到工作目录后再读取（不得凭记忆代替）**：初始化及每次任务/角色启动时，先把固定 12 个 `skills/<name>/SKILL.md` 的正文同步到工作文件区的技能缓存（本地/通用 `盯盘/skills/`，Coze 左侧工作文件 `skills/`），并**从工作目录读取内化**；文档版本变化时先刷新该缓存再执行任务。任一技能未落地或读取失败必须停止依赖该技能的动作并如实报告，禁止用 `index.md`、角色摘要或历史记忆猜测规则。技能缓存属于工作产物，不写入记忆区。
2. **定时任务中间产物一律不永久保存，只留最终汇总报告**：T1/T2/T3/W1/M1/P1 等定时任务中，子 Agent 的原始意见、逐接口返回、草稿、打分中间表、临时抓取的资讯原文等一切中间文件只允许写入工作文件根 `tmp/`，任务结束（成功或失败）后立即清理；**只有主 Agent 汇总后的最终报告**（日报 / 周报 / 月报 / 复盘）以及规则允许的业务快照（`daily` 审计、`log_selection`、predictions、学习日志）可以保存。子 Agent 不得把中间产物写入日期目录、`投研/`、`选股/` 或任何记忆目录。
3. **报告必须说人话、结论前置、不暴露过程**：面向用户的报告（尤其复盘、当日总结、研报、选股）首屏严格按「用户最想知道」的顺序前置——① 今天市场发生了什么（复盘结论、涨跌与量能、风险偏好）；② 板块行情与大环境（板块强度、轮动方向、择时顺势判断）；③ 明天该关注哪些股票（短线角度 + 趋势角度的核心股票、热点热门板块龙头、事件受益股）；④ 综合量化/择时/趋势/热点/短线的选股结果与选股理由。**严禁输出非结论性的思考过程、原始数据转储、不带中文解释的接口名/参数名/JSON 字段/因子代码**；这些只允许出现在报告末尾的「数据来源」附录、故障诊断或用户明确索取参数时，并紧邻中文解释。
4. **选股必须按当日热点经申万分级行业量化，并上传服务端**：正式选股（自动 T3/W1 与用户主动方向选股）不得无差别机械筛选全市场；必须先识别当日热点/主线/核心事件，映射到申万一级/二级/三级行业，再用 `screen_quant` 传入对应 `industries` 在行业范围内量化选股，配合 `screen_sector` 的行业强度。跑出的正式候选必须按 `log_selection` 规范上传服务端（自动 `category=auto`、用户主动 `category=manual`），携带同日 `screening_run_id`、`selected_at`、精炼 `core_event`、写清受益逻辑与量化依据的 `reason`、按“题材→细分→事件→固定属性”排序的 `tags`；报告正文必须写明每只候选的选股理由，上传失败要如实披露并保留待办，不得声称已上传。
...
### 第 3 步：完整加载全部 12 个 Skills（强制，先落地工作目录再读取）
首次初始化以及**每次任务启动、每次 Agent/角色启动**时，必须**先把以下固定 12 个 `SKILL.md` 正文同步到工作文件区的技能缓存**（本地/通用 `盯盘/skills/`，Coze 左侧工作文件 `skills/`），再从工作目录逐文件完整读取内化；不得只凭 `index.md`、角色摘要、历史记忆或主绑定清单代替正文。技能缓存是工作产物，不写入记忆区；文档版本变化时先刷新缓存再执行任务：
