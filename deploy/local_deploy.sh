...
#
# 用法（在仓库根目录）：
#   bash deploy/local_deploy.sh                # 推送当前分支并部署
#   BRANCH=main bash deploy/local_deploy.sh    # 指定分支
#
# 前提：
#   - 本地已配置到服务器的 SSH（建议用密钥免密）；
#   - 服务器已按 deploy/remote_deploy.sh 头部说明完成一次性准备（clone + .env）。
#
# 可用环境变量覆盖默认值：
#   SERVER_IP  SSH_USER  SSH_PORT  REMOTE_APP_DIR  BRANCH  SERVICE_NAME  PORT
# =============================================================================
set -euo pipefail

# SSH_HOST：若配置了 ~/.ssh/config 别名（如 aliyun-stock），优先用别名连接（端口/用户/密钥由 config 决定）
SSH_HOST="${SSH_HOST:-}"
...
if [ -n "$SSH_HOST" ]; then
  ...
  SSH_CMD=(ssh -p "$SSH_PORT" -o StrictHostKeyChecking=accept-new "$SSH_USER@$SERVER_IP")
  log "连接 $SSH_USER@$SERVER_IP:$SSH_PORT，执行远端部署脚本"
fi
"${SSH_CMD[@]}" \
  "APP_DIR='$REMOTE_APP_DIR' BRANCH='$BRANCH' PORT='$PORT' SERVICE_NAME='$SERVICE_NAME' \
   bash '$REMOTE_APP_DIR/deploy/remote_deploy.sh'" \
