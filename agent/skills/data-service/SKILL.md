...
- `precompute_status` 返回覆盖率、任务状态、错误信息和因子公式版本。T3 发现状态未成功或门禁未满足时，只披露缺口并按报告接口规则重试读取状态，不得把失败处理转换为预计算调用。
...
- `quant_watch_status`：管理员可按 `trade_date` 单次读取最近 30 个自然日内的服务端确定性扫描聚合结果；省略日期时返回最近有数据日。仅在用户当前明确请求盘中或历史盯盘分析时作为证据读取，不得由 Agent 轮询；必须区分请求日期、实际日期与历史标记。
- `quant_watch_get_config` / `quant_watch_set_config`：管理员盯盘设置；Agent 不主动修改。通知默认关闭，只有用户明确要求时才可协助单次配置。
- `quant_watch_scan_once`：仅限用户明确要求的盘中诊断；Agent 不自动调用、不作为失败回退。
- 盘中评分不写日终表、正式选股、预测或记忆；逐笔大单、分钟样本、申万层级缺失时必须保留不可用状态。

## v2.2.0 当前调度总则

仅 T1/T3/W1/M1/P1 可作为 Agent 定时任务。初始化删除旧 T6/T7/D1 及旧自动盯盘/预计算任务；服务端交易日 16:00 自动收口，Agent 只读状态。管理员 `precompute_daily_factors` 仅限用户当前明确要求的单次诊断或补数。

## 接口规范位置与报告接口问题约定（v2.6.0）

- 本技能是全部取数的统一底座；更完整的调用协议、五轨版本、错误码与功能分组交叉表已随初始化落地到工作目录：`工作文档/接口文档/AGENT_SERVICE_GUIDE.md`、`工作文档/接口文档/SERVICE_INDEX.md`，随时可查，无需猜测。各业务 Skill 内的「本技能接口速查」列出各自所用接口并指向此处。
- **报告中的接口问题一律置于文末**：定时任务与 Skill 生成的报告，正文只写面向用户的中文结论；接口失败/降级/缺数统一收敛到 output-format「🛠️ 数据接口问题」文末附录，标接口名称、功能、报错信息；**仅量化选股（`screen_quant`/`screen_trend`/`screen_sector`）与选股上传（`log_selection`）附请求参数**，其余接口不附。
