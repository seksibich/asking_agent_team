...
T1/T2/T3 关键数据接口 4xx/5xx/空数据：记录首次失败，首次失败后 5 分钟重试一次、15 分钟再重试一次。401 鉴权及明确参数/配置错误不盲目重试。最终仍失败时标降级与缺失并继续可完成部分；非关键接口失败不阻塞整份报告，禁止编造。竞价与盘中能力仅在用户明确请求时单次调用，不得注册自动任务或因失败重试而启动持续盯盘。
