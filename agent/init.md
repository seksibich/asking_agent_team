...
- **AGENT_DOC_VERSION：`v2.4.2`**（量化盯盘历史回看与前端默认入口调整，2026-07-21）
...
1. **量化盯盘支持短期历史回看**：`quant_watch_status` 可按 `trade_date` 读取最近 30 个自然日内仍保留的聚合消息；省略日期时返回最近有数据日。历史结果仍是对应交易日的盘中临时聚合，不得冒充完整日因子或正式选股。
2. **历史读取不改变 Agent 边界**：Agent 只在用户当前明确请求时单次读取，不轮询、不触发扫描；前端历史页断开 WebSocket，点击“最新”后才恢复实时模式。
3. **日期口径必须显式区分**：消费响应时区分请求日期、实际返回日期、当前上海日期和历史标记；所选日期无数据时如实披露，不用其他日期结果冒充。
