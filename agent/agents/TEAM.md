...
- **仅重量级任务启用团队**：盘前汇总（T1 08:30）、22:00 综合复盘（T3）、周/月回测、**用户主动发起的分析/选股任务**。
- **禁止 Agent 自动盯盘**：不注册竞价、解释型盘中扫描或午间总结定时任务。数据服务可独立运行确定性 `quant_watch` 扫描；团队不启动、不循环、不补跑，也不因其消息主动执行。用户明确请求竞价或盯盘时，由主 Agent 单次执行。（v2.7.0：已取消 17:30 当日总结 T2，交易日只保留 T1 08:30 与 T3 22:00。）
...
0. **中间产物不留存**：子 Agent 的原始意见、逐接口返回、草稿、中间打分表、临时抓取的资讯原文等一切中间文件只回传给主 Agent 或写入工作文件根 `tmp/`，定时任务结束（成功或失败）后立即清理；子 Agent 不得把中间产物写入日期目录、`投研/`、`选股/`、`周报/`、`月报/` 或任何记忆目录。只有主 Agent 汇总后的最终报告与规则允许的业务快照可以保存。
...
`stock-research` 是用户主动单股调研入口，**不加入定时 T1/T3 的必执行绑定**；但全员启动时仍必须完整加载。回测分析师不得将 watch 样本用于 auto 胜率、`tuning_hints` 或调参。

分发任务时必须在任务描述中点名对应 `skills/<name>/SKILL.md`；涉及取数统一执行 `skills/data-service/SKILL.md` 的 fallback、缺失标注与定时延迟重试规则。
## v2.2.0 当前团队边界

- 团队仅执行现行 **T1/T3/W1/M1/P1**：T1 为 08:30 团队盘前汇总，T3 为 22:00 团队综合复盘；**v2.7.0 已取消 T2（17:30 当日总结）**，旧 T6/T7/D1 必须在初始化时删除。
