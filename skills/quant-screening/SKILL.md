...
- 用户主动入口默认 `ephemeral`；仅用户明确要求持久化后为 `watch`，做 1/3/7/30 日观察性回测且永不进入 auto 胜率、`tuning_hints` 或调参。
...
8. 仅调度器正式自动候选可按规则写 `predictions.jsonl`、观察池并 `log_selection(category=auto)`。用户主动结果默认 `ephemeral`；仅用户明确要求加入观察/持续跟踪/纳入回测时转 `watch`，补齐题材事件字段并做隔离的 1/3/7/30 日观察性回测。业绩增长参考池本身不得触发任何持久化。
