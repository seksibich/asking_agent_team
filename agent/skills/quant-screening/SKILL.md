...
- **自动入口**：T3/W1 等调度任务由消息面、热榜、涨停连板、量能与资金聚类识别动态题材/具体事件，再映射候选并执行 `screen_quant` / `screen_trend`。
- **用户主动入口**：用户可指定具体事件、行业导向或热门板块；先扩展产业链与直接/间接受益候选，再执行相同量化筛选。题材可独立于传统行业板块，不得被申万、东财、同花顺分类限制。

### 热点 → 申万分级行业 → 量化选股（强制，禁止无差别全市场机械筛选）★

正式选股（自动 T3/W1 与用户主动方向选股）**不得对全市场无差别机械筛选**，必须走「热点主线 → 申万分级行业 → 行业内量化」的收窄路径：

1. **先定位当日热点/主线/核心事件**：综合消息面、热榜、涨停连板、量能与资金聚类，识别当天真正有资金和事件驱动的题材/主线。
2. **映射到申万分级行业**：把热点主线落到申万一级/二级/三级行业（必要时叠加同花顺/东财概念补充主题股），并用 `screen_sector` 的行业强度分位确认所选行业确实处于强势/轮动方向。
3. **在行业范围内量化**：调用 `screen_quant` 时通过 `industries` 传入定位到的申万分级行业词条（多词条取交集层层收窄），只在这些行业内做横截面量化，而不是省略 `industries` 跑全市场。仅当用户明确要求全市场普筛时才可省略 `industries`，并在报告中说明这是用户指定的全市场模式。
4. **热点为空时如实说明**：若当日无明确热点或行业强度普遍走弱，如实披露并给中性/降仓结论，不得为凑候选而机械全市场拉取。

一句话：**选股跟着当日热点走，经申万分级行业收窄，再量化，最后逐只讲清受益逻辑并上传服务端**。
...
### 选股数量
- **自动跑（定时任务 T3/W1 等由调度触发的选股）：`top_n=50`**。
- 手动/网页触发可自定义（网页默认 30）。
...
数据源：申万一级行业指数（`sw_daily` + `index_classify`），无权限则如实失败。服务端交易日 16:00 日终任务把全部行业原始因子、综合分和横截面分位写入 `daily_sector_scores`；`screen_sector` 优先读同交易日持久化结果，缺失才实时计算。个股因子由服务端通过申万成分映射写入 `industry_strength`。Agent 只读 `health.daily_finalize` / `precompute_status`，不得自动调用 `precompute_daily_factors`。
...
板块/指数上下文引用 `market_index` 时，允许数组或逗号字符串；失败、空或部分 code 缺失则逐 code 调 `market_daily(code,start,end)`，取最近记录并标 `degraded`/实际日期（数据接口间等价回退）。**数据类接口失败则失败、如实披露，禁止编造或臆造缺失因子**。候选分组所需新闻/公告不在数据服务，从各财经平台多源检索（≥2 来源交叉，标来源与时间）；全失败标“资讯面不可用 + 已尝试来源”，不得当作无风险。T3/W1/M1 中非关键接口失败不阻塞其余候选与报告。
