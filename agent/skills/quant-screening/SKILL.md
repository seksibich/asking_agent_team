...
{"function": "screen_quant",
 "params": {"industries": ["光伏", "半导体"], "stock_names": ["宁德时代", "贵州茅台"], "top_n": 30}}
```
...
- **输入宽容**：大小写不敏感、自动去首尾空格，中英文逗号(`，`/`,`)均可切分。
...
8. 调度器正式自动候选逐只 `log_selection(category=auto)`；用户触发正式选股候选逐只 `log_selection(category=manual)`。两类都必须传 `selected_price`（拿不到可省略，由服务抓最近收盘）、`hotspot`、`event`、`market_role`、完整 `reason` 与 `factors`（含行业强度、个股原始因子和综合分）。`manual` 只做隔离回测，不写自动 predictions/daily/观察池、不参与调参；普通研究和业绩增长参考池不得持久化。
